...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-activation-model-mixin runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-activation-model-mixin.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ activation_model_mixin")
...
def lint(c):
    ...
    """
    c.run("flake8 django-activation-model-mixin tests")
...
def release(c, bumpsize=''):
    ...
    import django_activation_model_mixin
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_activation_model_mixin.__version__))
    c.run("git push --tags")
