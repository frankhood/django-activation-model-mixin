...
class ModelAdminTest(TestCase):
    ...
    def test_utility(self):
        example_model_admin = ExampleModelAdmin(
            model=ExampleModel, admin_site=AdminSite()
        )
        self.assertIn("is_active", example_model_admin.list_display)
        self.assertIn("is_active", example_model_admin.list_filter)

        self.assertIn(
            "make_active", example_model_admin.get_actions(request=self.request)
        )
        self.assertIn(
            "make_inactive", example_model_admin.get_actions(request=self.request)
        )

        example = ExampleModel.objects.create()
        self.assertIn(
            "is_active",
            example_model_admin.get_fieldsets(request=self.request, obj=example)[1][
                1
            ].get("fields")[0],
        )
        self.assertIn(
        ...
        self.assertIn(
            "is_active",
            example_model_admin.get_readonly_fields(request=self.request, obj=example),
        )
        self.assertIn(
            "activation_date",
            example_model_admin.get_readonly_fields(request=self.request, obj=example),
        ...
        example_model_admin.make_active(
            request=self.request, queryset=ExampleModel.inactive_objects.all()
        )
        example.refresh_from_db()
        self.assertTrue(example.is_active)

        example_model_admin.make_inactive(
            request=self.request, queryset=ExampleModel.active_objects.all()
        )
        example.refresh_from_db()
