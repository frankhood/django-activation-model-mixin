...
class ActivationModelMixinAdmin(admin.ModelAdmin):
    ...
    fieldsets_moderation = ((_('Activation'), {'fields': (
    ...
    def __init__(self, *args, **kwargs):
        ...
        if (not self.SHOW_ACTIVATION) and self.change_form_template == ActivationModelMixinAdmin.change_form_template:
            logger.warning("Pay Attention! You have to set change_form_template=None "
    ...
    def get_fieldsets(self, request, obj=None):
        fieldsets = super(ActivationModelMixinAdmin, self).get_fieldsets(request, obj=obj)
        if self.SHOW_ACTIVATION_FIELDSETS:
            fieldsets = tuple(list(fieldsets) + list(self.fieldsets_moderation))
        return fieldsets

    def get_readonly_fields(self, request, obj=None):
        readonly_fields = super(ActivationModelMixinAdmin, self).get_readonly_fields(request, obj)
    ...
    def response_change(self, request, obj):
        ...
        if "_setactive" in request.POST:
            ...
            msg = format_html(_('The {name} "{obj}" was set as "active" \
                successfully'), **msg_dict)
        ...
        if "_setinactive" in request.POST:
            ...
            msg = format_html(_('The {name} "{obj}" was set as "inactive" \
                successfully'), **msg_dict)
            self.message_user(request, msg, messages.SUCCESS)
            redirect_url = request.path
            redirect_url = add_preserved_filters(
    ...
    def make_active(self, request, queryset):
        ...
        for obj in queryset:
            if obj.is_activable():
                obj.set_active(commit=True, request=request)
                updated_objects += 1
        message = "{updated_objects} Entry sono state attivate".format(**{'updated_objects': updated_objects})
    ...
    def make_inactive(self, request, queryset):
        ...
        for obj in queryset:
            if obj.is_deactivable():
                obj.set_inactive(commit=True, request=request)
                updated_objects += 1
        message = "{updated_objects} Entry sono state disattivate".format(**{'updated_objects': updated_objects})
        messages.info(request, message)
